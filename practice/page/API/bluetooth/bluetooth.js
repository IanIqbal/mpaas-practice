Page({
  ...
  openBluetoothAdapter() {
    my.openBluetoothAdapter({
      success: res => {
        if (!res.isSupportBLE) {
          my.alert({ content: 'i am sorry，Bluetooth on your mobile phone is temporarily unavailable' });
          return;
        }
  ...
  closeBluetoothAdapter() {
    my.closeBluetoothAdapter({
      success: () => {
        my.alert({ content: 'Bluetooth is turned off successfully！' });
      },
      fail: error => {
        my.alert({ content: JSON.stringify(error) });
      },
    ...
    my.getConnectedBluetoothDevices({
      success: res => {
        if (res.devices.length === 0) {
          my.alert({ content: 'No devices in connection！' });
          return;
        }
        my.alert({ content: JSON.stringify(res) });
    ...
    my.getConnectedBluetoothDevices({
      success: res => {
        ...
        my.readBLECharacteristicValue({
          ...
          fail: error => {
            my.alert({ content: 'read failure' + JSON.stringify(error) });
          },
        });
      },
    });
    ...
    my.getConnectedBluetoothDevices({
      success: res => {
        ...
        my.writeBLECharacteristicValue({
          ...
          value: 'ABCD',
          success: res => {
            my.alert({ content: 'Write data succeeded！' });
          },
          fail: error => {
            my.alert({ content: JSON.stringify(error) });
          },
      ...
      success: res => {
        ...
        my.notifyBLECharacteristicValueChange({
          ...
          success: () => {
            ...
            my.alert({ content: 'Listening succeeded' });
          },
          fail: error => {
            my.alert({ content: 'Listening failed' + JSON.stringify(error) });
          },
        });
      },
    });
  ...
  onBluetoothAdapterStateChange() {
    if (res.error) {
      ...
      my.alert({ content: 'Local Bluetooth status change：' + JSON.stringify(res) });
    }
  },
